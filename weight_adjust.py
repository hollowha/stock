...
final_prices = load_original_prices_aligned(config["final_price"], assets)
current_prices = infer_price_by_index(final_prices, Dataset, current_date_index) 
...
while current_date_index < len(Dataset.index)-1:
    ...
    stocks = convert_to_stocks(balance, pf_original.weights, current_prices)
    # print(stocks)
    last_prices = get_final_prices(current_prices, test_data)
    # print(last_prices)
    balance_from_stocks = calculate_portfolio_value(stocks, last_prices)
    print(balance_from_stocks)
    current_prices = last_prices
    print(pf_original.cumulative_return[-1]*balance)
    '''
...
plt.savefig(f"{config['output'][:-4]}_optimized_fee={total_fee}.png", bbox_inches="tight", pad_inches=0)
