...
def read_market_time(csv_path: str) -> List[pd.Timestamp]:
    """
    Read the marketTime CSV file containing listing dates for each asset.

    CSV format:
      - A single header row of asset names (order must match your assets list)
      - Exactly one data row: each cell is a listing date in 'YYYY-MM-DD'

    Args:
        csv_path: path to the CSV file

    Returns:
        A list of pandas.Timestamp objects, one per asset
    """
    df = pd.read_csv(csv_path)
    if df.shape[0] != 1:
        raise ValueError("CSV file must contain exactly one row of listing dates")

    # Convert the single row of strings to pandas.Timestamp and return as list
    market_times = pd.to_datetime(df.iloc[0]).tolist()
    return market_times
...
market_times = read_market_time(config["market_time"])
...
index_asset = Asset(daily_return=pd.Series(index[skipped_days:], index=pd.to_datetime(Dataset.index[skipped_days:])),
                    name=config["index"],
                    asset_type=ASSET_TYPE.MARKET_INDEX)
index_balance = 100000/index_asset.cumulative_return.iloc[0]                    
...
while current_date_index < len(Dataset.index)-1:
    ...
    ref_date = pd.to_datetime(test_dates[0])  
    unlisted_np = np.array([ref_date < md for md in market_times], dtype=bool)  
    unlisted_mask = torch.tensor(unlisted_np, device=device, dtype=torch.bool)
    ...
    balance_records.append([test_dates[0], balances.iloc[0], balances.iloc[-1]])
    balance_0050_records.append([f"0050_{split_from}_{split_to}", index_0050_balance*index_0050_asset.cumulative_return.iloc[current_date_index-skipped_days], index_0050_balance*index_0050_asset.cumulative_return.iloc[split_to_index-skipped_days]])

    balance = cumulative_value.iloc[-1]
    previous_pf = pf    
...
train_and_predict_for_combined_weight(train_data=all_train_data,
